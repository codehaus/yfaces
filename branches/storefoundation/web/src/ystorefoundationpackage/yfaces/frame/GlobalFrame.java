...
public class GlobalFrame extends AbstractYFrame {
	...
	private QuickSearchComponent createQuickSearchComponent() {
		...
		listener.setActionListener(super
				.createExpressionString("doQuickSearch"));
	...
	private AdBannerComponent createAdBannerComponent() {
		...
		// final Locale locale =
		// SfRequestContext.getCurrentContext().getSessionContext().getLocale();
		// final String suffix = Locale.GERMAN.equals(locale) ? "_de" : "";

		final String suffix = YStorefoundation.getRequestContext()
				.getSessionContext().getLanguage().getIsocode();

		final AdBannerMetaData flashBanner = result.addBanner("hybris",
				"images/banners/storefoundation_banner_hybris.swf", "");
		result.addBanner("voucher", "images/banners/banner_voucher_" + suffix
				+ ".gif", null);
		result.addBanner("sale", "images/banners/banner_sale_" + suffix
				+ ".gif", null);

		flashBanner.getParameters()
				.put("name", "storefoundation_banner_hybris");
	...
	public void doQuickSearch(final YEvent<QuickSearchComponent> event) {
		...
		((ProductTableSortEvent) ptCmp.getSortEvent().getListener())
				.setTerm(cmp.getSearchTerm());
