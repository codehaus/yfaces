 ...
 */
public class DefaultProductDetailComponent extends AbstractYModel implements
		ProductDetailComponent {

	@SuppressWarnings("unused")
	private static final Logger log = Logger
			.getLogger(DefaultProductDetailComponent.class.getName());
	...
	public DefaultProductDetailComponent() {
		...
		this.ehCustomerReview = super
				.createEventHandler(new CustomerReviewAction());
		this.ehAddToWishlist = super
				.createEventHandler(new AddToWishListAction());
	...
	public void validate() {
		if (this.product == null) {
			this.product = YStorefoundation.getRequestContext()
	...
	@Override
	public void refresh() {
		final SfSessionContext sessCtx = YStorefoundation.getRequestContext()
				.getSessionContext();
	...
	private void setLocalizedAttributes(final ProductModel p) {
		...
		if (p.getVariantType() != null) {
			...
			if (p instanceof VariantProductModel) {
				final ProductModel base = ((VariantProductModel) p)
						.getBaseProduct();
	...
	/**
	 * This event gets fired when the user tries to add the current product to
	 * the cart.
	 */
	public static class AddToCartAction extends
			DefaultYEventListener<ProductDetailComponent> {
		...
		public void actionListener(final YEvent<ProductDetailComponent> event) {
			...
			final AddToCartResult state = reqCtx.getOrderManagement()
					.addToCart(sessCtx.getCart(), cmp.getProduct(),
							cmp.getQuantity(), null);
			final String resource = reqCtx.getURLFactory().createExternalForm(
					TO_CART_LINK);
			final String toCartLink = "<a href=\""
					+ resource
					+ "\">"
					+ reqCtx.getContentManagement().getLocalizedMessage(
							"frames.productFrame.toCart") + "</a>";
			switch (state) {
			case ADDED_ONE:
				sessCtx.getMessages().pushInfoMessage("addedtocart_one",
						toCartLink);
				break;
			case ADDED_SEVERAL:
				sessCtx.getMessages().pushInfoMessage("addedtocart_many",
						String.valueOf(cmp.getQuantity()), toCartLink);
				break;
	...
	/**
	 * This event gets fired when the user tries to write a comment for the
	 * current product.
	 */
	public static class CustomerReviewAction extends
			DefaultYEventListener<ProductDetailComponent> {
	...
	/**
	 * This event gets fired when the user tries to add the current product to
	 * the active wish list.
	 */
	public static class AddToWishListAction extends
			DefaultYEventListener<ProductDetailComponent> {
		...
		public void actionListener(final YEvent<ProductDetailComponent> event) {
			...
			final boolean anonymousUser = JaloBridge.getInstance().isAnonymous(
					userSession.getUser());
			if (anonymousUser) {
				entries = (List<Wishlist2EntryModel>) FacesContext
				...
				entries = new ArrayList<Wishlist2EntryModel>(wishList
						.getEntries());
			}
			...
			while (it.hasNext()) {
				...
				if (entry.getProduct().getCode().equals(product.getCode())) {
					userSession.getMessages().pushInfoMessage(
							"components.productDetailCmp.alreadyInWishList",
							cmp.getProductName());
					return;
				}
			}
			...
			final String resource = sfCtx.getURLFactory().createExternalForm(
					TO_WISH_LIST_LINK);
			final String toWishListLink = "<a href=\"" + resource + "\">"
					+ wishList.getName() + "</a>";
			userSession.getMessages().pushInfoMessage(
					"components.productDetailCmp.addedToWishList",
					cmp.getProductName(), toWishListLink);
		}

		// only for anonymous user
		private Wishlist2EntryModel createEntry(final ProductModel product,
	 ...
	 */
	public static class PrintPageAction extends
			DefaultYEventListener<ProductDetailComponent> {
	...
	public boolean isVariantsAvailable() {
		return this.product.getVariantType() != null
				|| this.product instanceof VariantProductModel;
	}
	...
	public Prices getPricing() {
		if (this.pricings == null) {
			this.pricings = YStorefoundation.getRequestContext()
					.getOrderManagement().getPrices(this.product);
		}
	...
	public ProductFeatures getFeatures() {
		if (this.features == null) {
			// this.features =
			// JaloBridge.getInstance().getFeatures(this.product);
			this.features = YStorefoundation.getRequestContext()
					.getProductManagement().getFeatures(this.product);
		}
	...
	public String getQuantityInCart() {
		if (this.quantityInCart == null) {
			final SfSessionContext userSession = YStorefoundation
					.getRequestContext().getSessionContext();
			final Map<UnitModel, Long> quantityMap = getProductCount(
					userSession.getCart(), getProduct());
			this.quantityInCart = "0";
			if (!quantityMap.isEmpty()) {
				final ArrayList<String> entries = new ArrayList<String>();
				for (final Map.Entry<UnitModel, Long> entry : quantityMap
						.entrySet()) {
	...
	public boolean isCustomerReviewAvailable() {
		...
		// final Product p =
		// YStorefoundation.getRequestContext().getPlatformServices().getModelService().getSource(product);
		// final int number =
		// CustomerReviewManager.getInstance().getNumberOfReviews(p).intValue();
		// return (number > 0);
	}
	...
	private Map<UnitModel, Long> getProductCount(final CartModel cart,
		...
		if (!isBaseProduct) {
			...
			final List<AbstractOrderEntryModel> entries = JaloBridge
					.getInstance().getCartEntriesByProduct(cart, product);
			for (final AbstractOrderEntryModel entry : entries) {
				...
				final Long newQuantity = (oldQuantity != null) ? Long
						.valueOf(oldQuantity.longValue()
								+ entryQuantity.longValue()) : entryQuantity;
