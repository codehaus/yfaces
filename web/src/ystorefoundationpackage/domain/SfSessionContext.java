...
public interface SfSessionContext extends YSessionContext {
	...
	interface CatalogChangeListener {
		public void performCatalogChanged(CatalogModel oldCatalog,
				CatalogModel newCatalog);
	}

	/** Listener for product change events. */
	interface ProductChangeListener {
		public void performProductChanged(ProductModel oldProduct,
				ProductModel newProduct);
	}

	/** Listener for category change events. */
	interface CategoryChangeListener {
		public void performCategoryChanged(CategoryModel oldCategory,
				CategoryModel newCategory);
	}

	/** Listener for language change events. */
	interface LanguageChangeListener {
		public void performLanguageChanged(LanguageModel oldLanguage,
				LanguageModel newLanguage);
	}

	/** Listener for currency change events. */
	interface CurrencyChangeListener {
		public void performCurrencyChanged(CurrencyModel oldCurrency,
	...
	/**
	 * Returns the wish list for the current user. If the user does not have a
	 * wish list, one will be created.
	 * 
	...
	/**
	 * Returns the current session locale. As the session is available in view
	 * files the return value is very useful for locale sensitive components.
	 * 
	 * @return {@link Locale}
	 */
	Locale getLocale();
